...
export async function getSociety(): Promise<Society | null> {
  const response = await apiFetch<Society[] | PaginatedResponse<Society>>(`${BASE}/societies/`);
  const list = unwrapList(response);
  return list[0] ?? null;
}

export function getSocietySummary(societyId: number): Promise<SocietySummary> {
  return apiFetch<SocietySummary>(`${BASE}/societies/${societyId}/summary/`);
}

export function updateSociety(societyId: number, payload: Partial<Society>): Promise<Society> {
  return apiFetch<Society>(`${BASE}/societies/${societyId}/`, {
    method: 'PATCH',
    body: payload,
  });
}
...
export async function getWings(societyId?: number): Promise<Wing[]> {
  ...
  const response = await apiFetch<Wing[] | PaginatedResponse<Wing>>(`${BASE}/wings/${query}`);
  return unwrapList(response);
}

export function createWing(payload: { society: number; name: string }): Promise<Wing> {
  return apiFetch<Wing>(`${BASE}/wings/`, { method: 'POST', body: payload });
}

export function updateWing(id: number, payload: Partial<Pick<Wing, 'name'>>): Promise<Wing> {
  return apiFetch<Wing>(`${BASE}/wings/${id}/`, { method: 'PATCH', body: payload });
}

export function deleteWing(id: number): Promise<void> {
  return apiFetch<void>(`${BASE}/wings/${id}/`, { method: 'DELETE' });
}
...
export async function getFloors(wingId?: number): Promise<Floor[]> {
  ...
  const response = await apiFetch<Floor[] | PaginatedResponse<Floor>>(`${BASE}/floors/${query}`);
  return unwrapList(response);
}
...
export function createFloor(payload: {
  ...
  return apiFetch<Floor>(`${BASE}/floors/`, { method: 'POST', body: payload });
}
...
export function updateFloor(
  ...
  return apiFetch<Floor>(`${BASE}/floors/${id}/`, { method: 'PATCH', body: payload });
}

export function deleteFloor(id: number): Promise<void> {
  return apiFetch<void>(`${BASE}/floors/${id}/`, { method: 'DELETE' });
}

// ---- Flat Types ----

export async function getFlatTypes(): Promise<FlatType[]> {
  const response = await apiFetch<FlatType[] | PaginatedResponse<FlatType>>(`${BASE}/flat-types/`);
  return unwrapList(response);
}
...
export async function getFlats(filters?: {
  ...
  const response = await apiFetch<Flat[] | PaginatedResponse<Flat>>(`${BASE}/flats/${query}`);
  return unwrapList(response);
}
...
export function createFlat(payload: {
  ...
  return apiFetch<Flat>(`${BASE}/flats/`, { method: 'POST', body: payload });
}

export function updateFlat(id: number, payload: Record<string, unknown>): Promise<Flat> {
  return apiFetch<Flat>(`${BASE}/flats/${id}/`, { method: 'PATCH', body: payload });
}

export function deleteFlat(id: number): Promise<void> {
  return apiFetch<void>(`${BASE}/flats/${id}/`, { method: 'DELETE' });
}